...
\begin{tabular}{||c c c||} 
 \hline
  Method & LWF Accuracy & Training Dataset Size\\ [0.5ex]
 \hline\hline
 ...
 \hline
\end{tabular}
\caption{Accuracy based on the LWF benchmark and training data size of the pre-trained models \cite{sandberg}, \cite{geitgey}, \cite{amos2016}, \cite{deng2019}.}
\label{table:2}
\end{table}

The embeddings of FaceNet and ArcFace both have 512 features and the embeddings of Dlib and OpenFace have 128 features. The embeddings and corresponding labels (identity and profession) of are also saved. Table \ref{table:3} shows the dimensions of the feature embeddings extracted from each method.
...
\item \textbf{Profession Clustering: } The clustering algorithms from Section \ref{clustering} were applied to the feature embeddings. The four images in Figure \ref{fig:plot1} belong to two professions. After detecting the faces and representing them as embeddings, Figure \ref{fig:plot2} shows the clustering results. The features are reduced to 2-D embeddings by PCA for visualization.
\end{enumerate}
