...
Suppose $L = \{l_{1},...,l_{n}\}$ contains the actual labels for each face and $C = \{c_{1},...,c_{n}\}$ are the labels from the output of the clustering algorithm. Then for each face pair $(i, j)$ of each cluster, \\
The amount of true positives are the number of face pairs $(i, j)$ that are correctly clustered into the same cluster. $$TP = | \{(i, j) \ \textrm{where} \ c_{i} = c_{j} \ \textrm{and} \ l_{i} = l_{j}\}|$$
The amount of false positives is the number of face pairs $(i, j)$ that are incorrectly clustered to the same cluster: $$FP = | \{(i, j) \ \textrm{where} \ c_{i} = c_{j} \ \textrm{and} \ l_{i} \neq l_{j}\}|$$
The amount of true negatives is the number of face pairs that are clustered to a different cluster. $$TN = | \{(i, j) \ \textrm{where} \ c_{i} \neq c_{j} \ \textrm{and} \ l_{i} \neq l_{j}\}|$$

\cite{thesis}

For example if $P = \{(1, 2), (1, 3), (2, 3), (4, 5)\}$ is a set of face pairs from the ground truth labels and $Q = \{(2, 3), (1, 4), (1, 5), (4, 5)\}$ is the set of face pairs from the labels of the clustering algorithms where the face pairs are represented by the indices of their embeddings. Then
$$TP = | P \cap Q | = |\{(2, 3), (4, 5)\}| = 2$$
$$FP = |Q - P| = |\{(1, 4), (1, 5)\}| = 2$$
$$FN = |P - Q| = |\{(1, 2), (1, 3)\}| = 2$$
